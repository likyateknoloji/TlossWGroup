...
public class TlosSpaceWideBase {
	...
	public void startCpc() {
		...
		if (!getSpaceWideRegistry().isUserSelectedRecover()) {
			...
			try {
				...
				System.out.println(tlosProcessData.toString());
		...
		if (getSpaceWideRegistry().getCpcReference() == null) {
			...
			/**
			 * Otomatik olarak baþlatýlmayacak, ekrandan baþlat emrinin gelmesi
			 * beklenecek.
			 */
	...
	public boolean loadGlobalstateDefinitions() throws TlosRecoverException {
		...
		logger.info("");
		logger.info("### Çalýþma Durum Makinesi Tanýmlarý Yöneticisi ####");

		if (getSpaceWideRegistry().getGlobalStateDefinition() == null) {

			logger.info("Global durum tanýmlarýný yüklüyor...");

			String dbData = null;
			GlobalStateDefinition globalStateDefinition = DBUtils.getGlobalStateDefinitions();

			/**
			 * @author serkan Eðer persistence == true ise ve persisten edilen
			 *         dosya yok ise, sistem sanki persistent deðilmiþ gibi
			 *         davranýp veritabnýndan aldýðý bilgiler ile yoluna devam
			 *         eder. GlobalState tanýmlarý için bu þekilde bir davranýþ
			 *         görülmüþtür.
			 */
			...
			logger.info("Global durum tanýmlarýný yükledi !");
		}
	...
	public void shutDownHttpServer() {
		...
		try {
			...
			/**
			 * Aþaðýdaki süre, http server üzerinden gelen kapatma isteðinin
			 * cevabýný http serverýn hazmetmesi için gereken bir süre. Bu süre
