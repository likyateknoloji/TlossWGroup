...
public class AgentManager implements Runnable {
	...
	protected void initAgentList() {
		...
		SpaceWideRegistry.getGlobalLogger().info("Etmen listesini db'den alýyor...");
		swAgents = DBUtils.initAgentList();
		SpaceWideRegistry.getGlobalLogger().info("Agent listesini yükledi!");

		if (swAgents == null) {
			/**
			 * @author serkan TODO Yapýlacak iþler notuna bakýn !!
			 */
			// SpaceWideRegistry.getSpaceWideLogger().info("Etmen listesi veri tabanýndan okunamadý ya da tanýmlý etmen yok !!!");
			// SpaceWideRegistry.getSpaceWideLogger().info("TlosSW Etmensiz yapýda çalýþacak !");
			System.exit(-1);
		} else {
	...
	public synchronized void updateHeartBeatTime(JmxAgentUser jmxAgentUser) {
		...
		if (cacheAgent == null) {
			System.err.println("Agent bilgisi saklandýðý yerde bulunamadý : AgentID : " + jmxAgentUser.getAgentId());
			System.err.println("Saklanan bilgiler : " + cacheAgent);
			System.err.println("Beklenmedik bir hata oluþtu, server kapandý !");
			System.exit(-1);
		}

		// TODO
		// ilk ayaga kalkýs anýnýda (agentlarýn) burada hallet
		// lastheartbeattime = 0 ise bu ilk andir
		// soru agentlar çalýþýyor konumda olabilirlermi
		// yeni ayaða kalktýklarýnda sýfýr ayaða kalktýk veya
		// dolu ayaða kalktýk gibi bir þey geçseler!! recover olabilir
		...
		if (!cacheAgent.getOutJmxAvailable()) {
			...
			if (!cacheAgent.getJmxAvailable()) {
				...
				// TODO sahin
				// Agent'da calisan isleri kill et kuyruklarý bosalt
