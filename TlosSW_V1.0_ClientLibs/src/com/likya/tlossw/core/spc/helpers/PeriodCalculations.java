...
public class PeriodCalculations {
	...
	public static Date forward(JobProperties jobProperties) {
		...
		periodInfo.getCounter().add(new BigInteger("1"));
		
		Calendar stepCalendar = periodInfo.getStep();
		
		long periodOfRepeatance = stepCalendar.getTimeInMillis();
	...
	public static Date findRangedNextPeriod(JobProperties jobProperties) {
		...
		jobProperties.setJobPlannedEndTime(changeYMDPart(myDate, jobProperties.getJobPlannedEndTime()));
		jobProperties.setJobPlannedStartTime(changeYMDPart(myDate, jobProperties.getJobPlannedStartTime()));
