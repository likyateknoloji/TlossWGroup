	...
	public void fileChanged(FileChangeEvent arg0) throws Exception {
		if(fileListener.getPollingProperties().getPollingType() == PollingType.UPDATE) {
			...
			if(fileListener.getFileNameType() == FileNameType.FULL_TEXT) {
				...
				if(triggerFileWithPath.equals(arg0.getFile().getURL().getFile().substring(3))) {
					myLogger.info("\"" + arg0.getFile().getURL().getFile().substring(3) + "\" guncellendi");
					outputFile.write(DateUtils.getCurrentTimeWithMilliseconds() + " \"" + arg0.getFile().getURL().getFile().substring(3) + "\" guncellendi" + System.getProperty( "line.separator" ));
	...
	public void fileCreated(FileChangeEvent arg0) throws Exception {
		if(fileListener.getPollingProperties().getPollingType() == PollingType.CREATE) {
			...
			if(fileListener.getFileNameType() == FileNameType.FULL_TEXT) {
				...
				if(triggerFileWithPath.equals(arg0.getFile().getURL().getFile().substring(3))) {
					myLogger.info("\"" + arg0.getFile().getURL().getFile().substring(3) + "\" olusturuldu");
					outputFile.write(DateUtils.getCurrentTimeWithMilliseconds() + " \"" + arg0.getFile().getURL().getFile().substring(3) + "\" olusturuldu" + System.getProperty( "line.separator" ));
