...
public abstract class Job implements Runnable, Serializable {
	...
	protected synchronized void sendStartInfo(Date startTime) {
		...
		if (genericInfoSender != null) {
			...
			jobStart.setJobKey(getJobRuntimeProperties().getJobProperties().getBaseJobInfos().getJsName());
	...
	private synchronized void sendServerStatusChangeInfo() {
		...
		jobInfo.setJobKey(getJobRuntimeProperties().getJobProperties().getBaseJobInfos().getJsName());
